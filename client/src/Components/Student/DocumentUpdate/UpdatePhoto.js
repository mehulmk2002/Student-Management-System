...
const UpdatePhoto = (props) => {
  ...
  const handlePhotoUpload = async () => {
    ...
    try {
      const response = await axios.put(`http://localhost:4000/registration/students/${studentId}/photo`, formData, {
        headers: {
      ...
      props.closeModal()
      // You can update the UI or take any other necessary action here
    } catch (error) {
      console.error(error);
  ...
  return (
    <div>
      ...
      {imagePreviewUrl && (
        ...
        <div >
          <label htmlFor="upload_photo" style={{color:'#fff',cursor:'pointer'}} className="custom-file-upload">
